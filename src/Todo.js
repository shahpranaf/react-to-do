	...
	handleKeyDown = (e) => {
		...
		if ((e.keyCode === 38 || e.keyCode === 37) && cursor > 0) {
			this.setState(
				...
				document.getElementById(`check${this.state.cursor - 1}`).focus()
			);
	...
	getForm = (newTask) => (
		<form className="col-md-8 form-inline mb-2 row justify-content-between" onSubmit={this.handleSubmit}>
			...
			<button
				...
				className="btn btn-primary col-md-2 float-right"
				type="submit"
			>
				Add
			</button>
				...
				<div className="col-md-8">
					<div className="card-hover-shadow-2x mb-3 card">
						...
						<div className="d-inline-block d-flex card-footer">
							<span className="text-left col-md-6">{this.selectCount} Selected</span>
							<div className="text-right col-md-6">
								<button
									aria-label="deleteAll"
									onClick={this.handleDelete}
									className={`btn btn-danger ${!this.selectCount ? "invisible" : ""}`}
								>
									Delete
								</button>
							</div>
